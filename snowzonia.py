...
def gcloud_query():
	...
	os.system('rec -r 16000 -c 1 -b 16 -e signed-integer ' +
	          os.path.join(PATH, "query.wav") + ' silence -l 1 0.1 1% 1 0.5 1% trim 0 5')
...
if multiple_users:
    ...
    leave_sleep_mode_models = os.path.join(
        model_path_1, "leave_sleep_mode.pmdl")  # 1
...
player_lock = Lock()
command_timer = Process(
    target=command_timeout_or_playback_started_from_elsewhere, args=(player_lock,))
...
if multiple_users:
	wakewords = [wakeword_1, wakeword_2,
	    next_from_wakeword_detection, next_from_wakeword_detection]
else:
...
commands_detector_1=snowboydecoder.HotwordDetector(
    command_models_1, sensitivity=0.4, audio_gain=1)

if multiple_users:
	commands_detector_2=snowboydecoder.HotwordDetector(
	    command_models_2, sensitivity=0.4, audio_gain=1)

sleep_mode_detector=snowboydecoder.HotwordDetector(
    leave_sleep_mode_models, sensitivity=0.4, audio_gain=1)

wakeword_detector=snowboydecoder.HotwordDetector(
    wakeword_models, sensitivity=0.4, audio_gain=1)
